...
```bash
mkdir -p src/modules/sync
```

#### 9-3. 建立 `src/modules/sync/sync.service.ts`
...
export class SyncService {
  ...
  async syncHackmd(): Promise<void> {
    ...
    try {
      ...
      for (const note of notes) {
        try {
          // 查詢此筆記上次同步時間
          const lastSyncedAt = await this.supabaseService.getLastSyncedAt(
            "hackmd",
            note.id,
          );
          ...
          if (
            lastSyncedAt &&
            new Date(note.lastChangedAt) <= new Date(lastSyncedAt)
          ) {
            skipped++;
            continue;
          }
      ...
      await this.supabaseService.writeSyncLog("hackmd", "error", {
    ...
    try {
      ...
      for (const repo of repos) {
        try {
          ...
          if (
            lastSyncedAt &&
            new Date(repo.pushedAt) <= new Date(lastSyncedAt)
          ) {
            skipped++;
            continue;
          }
      ...
      await this.supabaseService.writeSyncLog("github", "error", {
        message: String(error),
      });
    }
  }
  ...
  async syncAll(): Promise<{ hackmd: unknown; github: unknown }> {
    ...
    await this.supabaseService.writeSyncLog("all", "success", {
      hackmd,
      github,
    });
...
#### 9-4. 補充 `supabase.service.ts`：新增兩個方法
...
async getLastSyncedAt(sourceType: string, sourceId: string): Promise<string | undefined> {
  const { data } = await this.client
    ...
    .order('updated_at', { ascending: false }) // ascending: false 降冪
    .limit(1)
    .single()
...
await this.supabaseService.writeSyncLog(
...
#### 9-5. 建立 `src/modules/sync/sync.module.ts`
...
> **注意**：`getTaggedRepos()`、`getRepoMarkdownFiles()` 和 `getFileContent()` 在 Phase 2 Step 7 建立 `github.service.ts` 時已一併包含，不需要再另外新增。
...
#### 10-3. 補充 `ingestion.service.ts`：新增單檔案處理方法

打開 `src/modules/ingestion/ingestion.service.ts`，在 `ingestAllGithubRepo` 方法**後面**加入：
...
> `getFileContent` 在 Phase 2 Step 7 的 `github.service.ts` 已包含，無需另外新增。

#### 10-4. 更新 `sync.module.ts`：加入 Controller
...
#### 10-5. 在 `app.module.ts` 引入 SyncModule 和 ScheduleModule
...
#### 10-7. 在 GitHub 設定 Webhook（每個 repo 都要設定一次）
